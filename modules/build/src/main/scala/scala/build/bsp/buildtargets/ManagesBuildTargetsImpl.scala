...
//  protected val generatedSources = mutable.Map[Scope, GeneratedSources]()
  ...
  override def targetIds: List[b.BuildTargetIdentifier] = managedTargets.values.toList.map(_.targetUri)
  ...
  override def addTarget(projectName: ProjectName, workspace: os.Path, scope: Scope, generatedSources: Seq[GeneratedSource] = Nil): Unit =
  ...
  override def setGeneratedSources(projectName: ProjectName, sources: Seq[GeneratedSource]): Unit = {
